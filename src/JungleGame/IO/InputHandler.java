...
public class InputHandler {
    ...
    private String NextToken() {
        ...
        // while the given part is not a token, get more.
        while (next != ' ') {
            temp.append(next);
            if (sb.length() == 0 || Keyword.isKeyword(temp.toString()) || Keyword.isCoordinate(temp.toString())) {
                break;
            }
            next = sb.charAt(0);
            sb.deleteCharAt(0);
    ...
    public static void main(String[] Args) {
        ...
        System.out.println(getInputHandler().getCommand() instanceof GameCommand);
...
enum Keyword {
    Key_Move("Move"), Key_Save("Save"), Key_Load("Load"), Key_Pointer("->");
